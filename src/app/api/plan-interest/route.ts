...
export async function POST(request: Request) {
  ...
  if (await consumeDistributedRateLimit({ key: `interest:${uid}`, limit: 10, windowMs: 10 * 60_000 }) === 'limited') return NextResponse.json({ error: 'rate-limited' }, { status: 429, headers: NO_STORE_HEADERS });
