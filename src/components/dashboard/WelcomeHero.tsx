...
export default function WelcomeHero({ wordsToday, msToday, streak, onStart }: Props) {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-blue-950/60 via-zinc-900 to-black/90 px-6 py-8 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_20px_50px_rgba(0,0,0,0.35)] md:px-10">
      ...
      <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="max-w-xl">
          ...
          <p className="mt-1.5 text-sm leading-relaxed text-slate-400">
            Loop, repeat and retain vocabulary hands-free. A few focused minutes
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-medium text-slate-200">
            <span aria-hidden>🎧</span> {wordsToday} word{wordsToday === 1 ? '' : 's'} today
          </span>
          <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-medium text-slate-200">
            <span aria-hidden>⏱</span> {formatDuration(msToday)} studied
          </span>
          <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-medium text-neon-amber">
            <span aria-hidden>🔥</span> {streak > 0 ? `${streak} day${streak === 1 ? '' : 's'}` : 'Start a streak'}
          </span>
          {onStart && (
            <button
              onClick={onStart}
