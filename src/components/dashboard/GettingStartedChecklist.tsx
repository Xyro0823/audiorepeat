...
export default function GettingStartedChecklist({
  ...
  const tasks: { id: string; labelKey: TKey; done: boolean; icon: typeof Languages; action: () => void; actionLabelKey: TKey; disabled?: boolean }[] = [
    { id: 'language', labelKey: 'dashboard.checklist.language.label', done: languageReady, icon: Languages, action: onChooseLanguage, actionLabelKey: 'dashboard.checklist.language.action' },
    { id: 'sets', labelKey: 'dashboard.checklist.sets.label', done: setReady, icon: LibraryBig, action: onBrowse, actionLabelKey: 'dashboard.checklist.sets.action' },
    { id: 'practice', labelKey: 'dashboard.checklist.practice.label', done: practiceReady, icon: Headphones, action: onPlay, actionLabelKey: 'dashboard.checklist.practice.action', disabled: !canPlay },
  ];
  const progressText = t('dashboard.checklist.progress', { done: complete, total: 4 });

  return (
    <section className="glass animate-fade-up relative mt-5 rounded-3xl p-4 sm:p-5" aria-labelledby="getting-started-title">
      <button type="button" onClick={dismiss} aria-label={t('dashboard.checklist.dismissAria')} className="absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-full text-slate-400 transition hover:bg-white/5 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
        <X className="h-4 w-4" aria-hidden />
      </button>
      <div className="pr-11 sm:flex sm:items-end sm:justify-between sm:gap-5">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-neon-cyan">{t('dashboard.checklist.kicker')}</p>
          <h2 id="getting-started-title" className="mt-1 text-lg font-semibold text-white">{t('dashboard.checklist.title')}</h2>
          <p className="mt-1 text-xs text-slate-400">{progressText}</p>
        </div>
        <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-white/10 sm:mt-0 sm:max-w-48" aria-label={progressText}>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {tasks.map(({ id, labelKey, done, icon: Icon, action, actionLabelKey, disabled }) => (
          <div key={id} className={`rounded-2xl border p-3 ${done ? 'border-emerald-400/20 bg-emerald-400/[0.06]' : 'border-white/10 bg-white/[0.035]'}`}>
            <div className="flex items-center gap-2.5">
              ...
              <span className={`text-xs font-semibold ${done ? 'text-emerald-200' : 'text-slate-200'}`}>{t(labelKey)}</span>
            </div>
            {!done && (
        ...
        <div className={`rounded-2xl border p-3 ${installed ? 'border-emerald-400/20 bg-emerald-400/[0.06]' : 'border-white/10 bg-white/[0.035]'}`}>
          <div className="flex items-center gap-2.5">
            ...
            <span className={`text-xs font-semibold ${installed ? 'text-emerald-200' : 'text-slate-200'}`}>{t('pwa.install.button')}</span>
          </div>
