...
export default function StarterLibraryModal({ sets, onClose, onImport }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Browse starter sets"
    >
      ...
      <div className="glass animate-fade-up relative flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl">
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          <div>
            <h2 className="text-xl font-bold text-white">Browse starter library</h2>
            <p className="text-xs text-slate-400">
              Pre-built CEFR-leveled sets · pick a language & level, then import
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            {CEFR_LEVELS.map((lvl) => (
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                  level === lvl ? CEFR_META[lvl].chip : 'border-white/10 text-slate-400 hover:border-white/25 hover:text-white'
                }`}
              >
                {lvl}
              </button>
        ...
        <div className="grid flex-1 gap-3 overflow-y-auto p-6 pt-4 sm:grid-cols-2">
          {filtered.length === 0 ? (
            <div className="col-span-full rounded-2xl border border-white/10 p-8 text-center text-sm text-slate-400">
              No starter sets match{lang !== 'all' ? ` for ${starterLangLabel(lang)}` : ''} — try
              another language or level.
            </div>
          ) : (
            filtered.map((starter) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-slate-400">{CEFR_META[starter.level].description}</p>
